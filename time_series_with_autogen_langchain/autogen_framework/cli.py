"""Autogen CLI entrypoint: collect inputs and delegate to processor only."""
...
from .processor import AutogenProcessor, AutogenProcessorConfig
...
def _collect_ticker(raw: str | None = None) -> str:
    ...
    while True:
        raw_val = (_ask("Ticker", raw) if raw else _ask("Ticker", None)).strip().upper()
...
def run() -> None:
    parser = argparse.ArgumentParser(description="Autogen LLM-orchestrated time-series CLI")
