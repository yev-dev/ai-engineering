...
from .react_engine import AutogenReActEngine
from .tools import build_autogen_tool_registry


@dataclass
class AutogenProcessorConfig:
    ticker: str
    start: date
    end: date
    source: str | None
...
class AutogenProcessor:
    def __init__(self, config: AutogenProcessorConfig, console: Console | None = None) -> None:
        self.config = config
    ...
    def _build_tools(self, run_id: str) -> CommonToolbox:
        ...
        market_data_agent = build_default_market_data_agent(use_live_yahoo=use_live_yahoo, seed=1001)
    ...
    def execute(self) -> None:
        ...
        run_id = self.config.run_id or generate_run_id(prefix="autogen")
        toolbox = self._build_tools(run_id=run_id)
        tool_registry = build_autogen_tool_registry(toolbox)
        ...
        orchestrator = LLMOrchestrator(llm_client=llm_client, framework="autogen", max_steps=20)
        react = AutogenReActEngine()
        ...
        for _ in range(orchestrator.max_steps):
            ...
            react.step(
                ...
                action=f"Tool call: {decision.tool} args={decision.args}",
        ...
        self.reporter.print_react_trace(react.events, "Autogen ReACT Trace")
