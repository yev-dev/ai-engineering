...
```python
from market_data_ai.time_series_construction import (
...
- `market_data_ai/validation_rules.example.json`
...
- `market_data_ai/.env/llm.defaults.env`
- `market_data_ai/.env/llm.local.env` (optional local override)

Use `market_data_ai/.env/llm.local.env.example` as a template for local/provider-specific overrides.

### Credentials from .env Files (Security)

API keys are **never committed** to `config.yaml`. Instead, credentials are sourced
from git-ignored `.env` files loaded automatically at startup:

- `market_data_ai/.env`
- `market_data_ai/.env.local` (optional, overrides the above)

The application loads these files in order (later files win) and resolves the
provider API keys as follows:
...
Add your real keys to `market_data_ai/.env`:
...
override per-run with `export DEEPSEEK_API_KEY=...` or `export GITHUB_TOKEN=...`.
The `config.yaml` `api_key` fields remain empty placeholders; if a value is
present there it takes precedence over the environment.
...
```bash
python -m market_data_ai.cli --list-model-config
```
...
TIME_SERIES_VALIDATION_RULES="market_data_ai/validation_rules.example.json" \
python -m market_data_ai.cli \
  --request "AAPL from 2023-01-01 to 2024-01-01"
```
...
LLM_MODEL="ollama/qwen2.5:1.5b" \
TIME_SERIES_VALIDATION_RULES="market_data_ai/validation_rules.example.json" \
python -m pytest market_data_ai/tests/test_cli_llm_integration_manual.py -q
```
...
LLM_MODEL="ollama/qwen2.5:1.5b" python -m market_data_ai.cli \
  --request "AAPL from 2023-01-01 to 2024-01-01"
```
...
```bash
pytest market_data_ai/tests/test_workflow_int.py -v
```
...
- recording of all steps, when do we have more than 1 iteration



i want to create dashbaord at @/market_data_ai/dashboard.py  that offer UI components with @/market_data_ai/dashboard_app.py  as backend that will allow me to run market_data_ai operations offering alternative to run it via @/market_data_ai/cli.py. Similiar to current dashboard implementation i want to be able to select model provider that would populate list of available models to select from available LLM models using @/market_data_ai/models.py . Models selection should be put in the left panel. In the main panel i want to have a drop down box to select from  PROMPT_REGISTRY in @/market_data_ai/prompt_library.py A selection from this drop down is optional, below i want to have a text box that can be either free text or if populated from drop down box populate it. Below this window i want to have a drop down box to select from availbale agents with run next to it. When you select from an egent, there should be expandable box to open to read definition from of selected agent filled when selected with definition in @/time_series_autogen/agents_definition.py . Once an agent is selected and we run to trigger the same way that @/market_data_ai/cli.py to trigger. All backend logic for dashboard UI should be in dashboard_app.py to have a clean separation between presentation layer and backend. Add progression bar below this button that show execution progress. Below we should have 
