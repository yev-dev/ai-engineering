...
from .handler import TimeSeriesConstructionHandler
...
from .tool_models import validate_tool_input
...
class TimeSeriesConstructionProcessor:
    ...
    def __init__(
        ...
        self.data_source_registry = data_source_registry or DataSourceRegistry()
        self.pending_events: list[CallbackEvent] = []
        logger.info(
            "processor_initialized session_id=%s data_files=%d",
            self.handler.session_id,
            len(self.data_source_registry),
        )
    ...
    def _prompt(self, agent: Agent) -> str:
        """Build the system prompt for an agent.

        Appends the available data files from the injected DataSourceRegistry
        so the LLM knows which registered files are available for the workflow.
        """
        base_prompt = agent_system_prompt(agent)
        if self.data_source_registry.is_empty:
            return base_prompt
        return f"{base_prompt}\n\n{self.data_source_registry.describe()}"
