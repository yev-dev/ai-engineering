...
from .models import ModelRequestFactory
...
class AutogenProcessorRuntime(AgenticRuntime):
    ...
    def __init__(
        ...
        self.processor = TimeSeriesConstructionProcessor(
            factory=factory,
    ...
    def active_sources(self) -> list[str]:
        """Return the active data sources carried by this runtime.

        Delegates to the injected :class:`DataSourceRegistry` (the database-
        backed carrier of file/source metadata).  Returns ``[]`` when no
        registry is attached.

        Returns:
            Sorted list of active source names (e.g. ``["yahoo"]``).
        """
        if self.data_source_registry is None:
            return []
        return self.data_source_registry.active_sources()
...
def build_runtime(
    ...
    if key == "autogen":
        return AutogenProcessorRuntime(
