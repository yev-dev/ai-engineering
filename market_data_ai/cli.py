...
from market_data_ai.agents_definition import CallbackEventType
from market_data_ai.configuration import get_config
from market_data_ai.database import (
    get_datastore,
    init_datastore,
)
from market_data_ai.models import ModelRequestFactory
from market_data_ai.prompt_library import (
    format_prompt_menu,
    resolve_prompt_selection,
)
from market_data_ai.runtime import AgenticRuntime, build_runtime
from market_data_ai.workflow_report import (
...
def _configure_logging(verbose: bool = False) -> None:
    """Configure application logging to the console and the configured log file.

    Logs are written both to the terminal and to
    ``config.paths.log_dir/market_data_ai.log``
    (e.g. ``~/market_data/logs``) so that all application log output is
    persisted on disk via the central configuration (``config.yaml``) rather
    ...
    log_file = log_dir / "market_data_ai.log"
    ...
    # Reset any pre-existing handlers so repeated calls never stack duplicates.
...
def _save_artifacts(
    ...
    try:
        ...
        try:
            datastore = init_datastore(
                config.paths.database_dir / "datastore.db"
            )
    ...
    rules_path = config.paths.validation_rules
    if rules_path:
        try:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=get_config().runtime.agentic_framework,
    ...
    # Configure logging (console + configured log_dir) before any workflow
    # work, using the central config so log files live under paths.log_dir.
    _configure_logging(verbose=args.verbose)

    # Apply runtime model overrides via configuration so the factory resolves
    # configuration in one place (from config.yaml).
    from market_data_ai.configuration import set_runtime_overrides
    ...
    if args.debug_flow:
        runtime_overrides["debug_flow"] = True
    if llm_overrides or runtime_overrides:
        set_runtime_overrides(
            **({"llm": llm_overrides} if llm_overrides else {}),
            **({"runtime": runtime_overrides} if runtime_overrides else {}),
        )
    ...
    if get_config().runtime.debug_flow:
        print("Debug flow: enabled")
    print(f"{'='*60}\n")
    ...
    logger.info(f"\nArtifacts saved to: {get_config().paths.output_dir / run_id}")
