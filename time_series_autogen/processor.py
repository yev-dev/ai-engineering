...
from time_series_autogen.agents_definition import (
    Agent,
    CallbackEvent,
    CallbackEventType,
    get_agent,
...
from time_series_autogen.models import LLMRequest, ModelRequestFactory
from time_series_autogen.tools import get_tool
from time_series_autogen.prompts import agent_system_prompt, request_prompt, unavailable_message
...
class TimeSeriesConstructionProcessor:
            ...
            return [
                CallbackEvent(
                    CallbackEventType.AWAITING_USER_INPUT,
                    {
                        "agent": "System",
                        "prompt": (
                            "The conda environment is configured outside the application. "
                            "Enter a financial request, for example: "
    ...
    def _recover_orchestrator_delegation(
        ...
        if target is None:
            return None
        logger.warning(
            "orchestrator_delegation_recovered target=%s iteration=%d",
